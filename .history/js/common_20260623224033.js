...
document.addEventListener('DOMContentLoaded', function() {
  ...
  formConfigs.forEach(config => {
    ...
    if (form) {
      form.addEventListener('submit', function() {
        submitted = config.successId; // どのサンクス表示を出すか記憶
      });
    }
  });
  ...
  if (iframe) {
    iframe.onload = function() {
      if (submitted) {
        // 現在送信されたフォームを隠し、対応するメッセージを表示
        const formElement = document.querySelector(`form:not([style*="display: none"])`); 
